    ...
    public function _main(Request $request)
    {
                    ...
                    if ($minutos >= $min_from && $minutos <= $min_to) {
                        ...
                        if ($user_pos > $i) {
                            ...
                            if ($stop->country == 'Cuba') {
                                ...
                                ), [$mapImagePath, $this->pathToService."/image/santa.jpg"]);
                                return $response;
                            } else {
                                $response = new Response();
		...
		if (($today == $the_day && $hora >= $santa_stops[0]->hour) || ($today > $the_day && $today <= $end_year))
		{
			...
			$response->createFromTemplate("basic.tpl", array(
				...
				"image" => "go.jpg",
			), [$this->pathToService."/image/go.jpg", $this->pathToService."/image/santa.jpg"]);
			return $response;
		}
        ...
        $response->createFromTemplate('basic.tpl', array(
            ...
            "image" => "sleeping.jpg",
        ), [$this->pathToService."/image/sleeping.jpg", $this->pathToService."/image/santa.jpg"]);
